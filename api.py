...
from services import query_creditcard
...
class SMSRequest(BaseModel):
    sms_text: Optional[str] = Field(None, alias='sms')
...
async def process_sms(request: SMSRequest):
    ...
    """
    try:
        if not request.sms_text:
            raise HTTPException(status_code=400, detail="No SMS text provided")

        # Log the received SMS for debugging (optional)
        print(f"Received SMS: {request.sms_text[:50]}...")

        # Check SMS relevance before parsing
        if not is_relevant(request.sms_text, request.sender):
            print(f"Discarded irrelevant SMS: {request.sms_text[:50]}...")
            return {"status": "discarded", "message": "SMS discarded as irrelevant"}

        # Process SMS with parser and database manager
        result = sms_parser.parse_sms(request.sms_text, request.sender, db_manager)

        # Return parsed data or minimal acknowledgement
        if result.get("status") == "discarded":
            return {"status": "discarded", "message": "SMS discarded by light filter"}
        else:
            return result  # Or return a success message: {"status": "success", "message": "SMS processed"}
...
async def enqueue_sms(request: SMSRequest):
    ...
    try:
        ...
        if background_service.is_running():
            # Add to queue if service is running
            enqueued = background_service.enqueue_sms(request.sms_text)
            return {
                "status": "success" if enqueued else "duplicate",
                "message": "SMS added to processing queue" if enqueued else "SMS already processed"
            }
        else:
            # Process immediately if service is not running
            print("Background service not running, processing SMS immediately")
            result = sms_parser.parse_sms(request.sms_text, request.sender, db_manager)
            return result

    except Exception as e:
        print(f"Error enqueuing or processing SMS: {e}")
